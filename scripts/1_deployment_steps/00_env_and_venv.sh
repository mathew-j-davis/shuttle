#!/bin/bash
# 00_env_and_venv.sh - Set up Shuttle environment variables and optionally create venv
#
# Usage: 
#   ./00_env_and_venv.sh                    # Use system production defaults (for services)
#   ./00_env_and_venv.sh -e                 # Use development defaults
#   ./00_env_and_venv.sh -u                 # Use user production defaults
#   ./00_env_and_venv.sh --do-not-create-venv  # Skip venv creation (any mode)
#   ./00_env_and_venv.sh [config] [venv] [work]  # Use custom paths
#
# Flags can be combined: ./00_env_and_venv.sh -e --do-not-create-venv
...
# Generated by 00_env_and_venv.sh on $(date)
...
if [[ "$CREATE_VENV" == true ]]; then
    ...
    if ! command -v python3 &>/dev/null; then
        ...
        echo "Please run 02_sudo_install_python.sh first."
        exit 1
    fi
