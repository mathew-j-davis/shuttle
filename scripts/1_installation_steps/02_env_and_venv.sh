...
SHUTTLE_LOG_LEVEL=DEBUG"
    ...
    # Use the standard helper to write .env file
    if write_file_with_sudo_fallback "$DOT_ENV_PATH" "$DOT_ENV_CONTENT" "true"; then
        echo "Development .env file created at: $DOT_ENV_PATH"
    else
        echo "Warning: Could not create .env file at: $DOT_ENV_PATH"
    fi
fi
